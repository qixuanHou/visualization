...
def entropy(class_y):
    ...
    probOne = float(nOne)/ float(nOne + nZero)
    probZero = float(nZero) / float(nOne + nZero)

    if probOne != 0 and probZero != 0:
        entropy = (-1 * probOne *np.log2(probOne))-(probZero *np.log2(probZero))
    else:
        entropy = 0
    return entropy
...
def information_gain(previous_y, current_y):
    ...
    prevE = entropy(previous_y)
    curE = (entropy(current_y[0]) * (len(current_y[0])/float(len(previous_y)))) + (entropy(current_y[1]) * (len(current_y[1])/float(len(previous_y))))

    info_gain = prevE - curE

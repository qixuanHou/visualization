...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        new_X = [row[:-1] for row in XX]
        new_y = [row[-1] for row in XX]
        from random import randint

        for i in range(n):
            sx = randint(0, len(new_X)-1)

            samples.append(new_X[sx])
            labels.append(new_y[sx])
    ...
    def fitting(self):
        ...
        while (i < len(self.bootstraps_datasets)):
